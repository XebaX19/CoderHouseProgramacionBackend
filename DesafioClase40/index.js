...
if (!cluster.isPrimary || MODE === 'FORK') {
  ...
  const { getAllProductosService, saveProductoService } = require('./services/productos.service');  
  ...
  app.use(session({
    name: 'session-desafio-40', //nombre que le doy a la cookie que utiliza la session para identificarla (por defecto se genera una cookie "connect-id" o algo así...) 
  ...
  app.post('/logout', async (req, res) => {
    try {
      ...
      req.session.destroy(err => {
        res.clearCookie('session-desafio-40'); //al eliminar la cookie, se eliminan todos los datos de la session. El nombre de la cookie es la que definí en el Middleware de la Session 
        if (err) {
          console.log(err);
        }
        else {
  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('nuevo-producto', async (prod) => {
      await saveProductoService(prod.title, +prod.price, prod.thumbnail);
