...
const { getProducto, getProductos, createProducto, updateProducto, deleteProducto } = require('./graphql/resolvers.js');
...
if (!cluster.isPrimary || MODE === 'FORK') {
  ...
  const { getAllProductosTestService, saveProductoTestService } = require('./services/productos-test.service');  
  ...
  app.use(session({
    name: 'session-desafio-42', //nombre que le doy a la cookie que utiliza la session para identificarla (por defecto se genera una cookie "connect-id" o algo así...) 
  ...
  app.post('/logout', async (req, res) => {
    try {
      ...
      req.session.destroy(err => {
        res.clearCookie('session-desafio-42'); //al eliminar la cookie, se eliminan todos los datos de la session. El nombre de la cookie es la que definí en el Middleware de la Session 
        if (err) {
          console.log(err);
        }
        else {
  ...
  io.on('connection', async (socket) => {
    ...
    const productos = await getAllProductosTestService();
    ...
    socket.on('nuevo-producto', async (prod) => {
      await saveProductoTestService(prod.title, +prod.price, prod.thumbnail);
