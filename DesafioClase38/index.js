...
if (!cluster.isPrimary || MODE === 'FORK') {
  ...
  app.use(session({
    name: 'session-desafio-38', //nombre que le doy a la cookie que utiliza la session para identificarla (por defecto se genera una cookie "connect-id" o algo así...) 
  ...
  app.post('/logout', async (req, res) => {
    try {
      ...
      req.session.destroy(err => {
        res.clearCookie('session-desafio-38'); //al eliminar la cookie, se eliminan todos los datos de la session. El nombre de la cookie es la que definí en el Middleware de la Session 
        if (err) {
          console.log(err);
        }
        else {
